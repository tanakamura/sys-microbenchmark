...
static void build_count_tree(hwloc_obj_t node, std::unique_ptr<count_node> &loc) {
    //printf("%p : %s (%s), arity=%d, osidx=%d\n", node, node->name, hwloc_obj_type_string(node->type), node->arity, node->os_index);
...
bind_self_to_1proc(ProcessorTable const *tbl, ProcessorIndex idx, bool membind)
{
    ...
    if (membind) {
        hwloc_set_membind(tbl->topo, idx.pu_obj->cpuset, HWLOC_MEMBIND_BIND, HWLOC_MEMBIND_THREAD | HWLOC_MEMBIND_NOCPUBIND);
    }
}
