...
static double run1(ThreadInfo *ti,
                   int nthread,
                   uint64_t (*func)(uint64_t zero, oneshot_timer *ot)
    )
{
...
struct ActualFreq
    :public BenchDesc
{
    typedef Table2D<double,std::string,uint32_t> table_t;

    ActualFreq()
        :BenchDesc("actual-freq")
    {}
...
#define ADD_LABEL(name,cond)                                                \
        ...
        table_t *result_table(new table_t("instruction","num_thread",row_label.size(),threads.size()));
...
#define DO_RUN(name,cond) if(cond()){ r = run1(&thread_info_list[0], tc, name); (*result_table)[row++][ti] = r/1e9; }
